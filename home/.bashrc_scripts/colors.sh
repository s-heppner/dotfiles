...
#
# Switch palette live with the `bash-color-theme` command (see bottom of file).
...
_BCT_DEFAULT_THEME="nord"

# The 16 base16 slots, in order. Used for iteration.
_BCT_SLOTS="00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F"
...
_bct_apply() {
    ...
    _bct_apply_ls_colors "$BASE05" "$BASE09" "$BASE0A" "$BASE0C" "$BASE0D"
}
...
_bct_apply_ls_colors() {
    local fg="$1" exec="$2" dir="$3" link="$4" orphan="$5"
    local c_fg c_exec c_dir c_link c_orphan
    c_fg="38;2;$(_bct_hex_to_rgb "$fg")"
    c_exec="38;2;$(_bct_hex_to_rgb "$exec")"
    c_dir="38;2;$(_bct_hex_to_rgb "$dir")"
    c_link="38;2;$(_bct_hex_to_rgb "$link")"
    c_orphan="38;2;$(_bct_hex_to_rgb "$orphan")"
...
_bct_usage() {
    cat <<'EOF'
bash-color-theme — switch the active base16 terminal palette

usage:
  bash-color-theme [list]        list installed palettes (* marks the active)
  bash-color-theme current       print the active palette name
  bash-color-theme set <name>    switch to <name> (persists across shells)
  bash-color-theme <name>        shorthand for `set <name>`
  bash-color-theme help          show this help
...
bash-color-theme() {
    ...
    case "$cmd" in
        ...
        set)
            if [ -z "${2:-}" ]; then
                printf 'usage: bash-color-theme set <name>\n' >&2
                return 2
            fi
            _bct_set "$2"
            ;;
        help|-h|--help)
            ...
            ;;
        *)
            # Shorthand: `bash-color-theme <name>` == `set <name>`.
            _bct_set "$cmd"
            ;;
    esac
}
...
_bct_complete() {
    ...
    if [ "$COMP_CWORD" -eq 1 ]; then
        COMPREPLY=( $(compgen -W "list current set help $(_bct_list)" -- "$cur") )
    elif [ "$prev" = "set" ]; then
        COMPREPLY=( $(compgen -W "$(_bct_list)" -- "$cur") )
    fi
}
complete -F _bct_complete bash-color-theme

# --- ls / grep color aliases -----------------------------------------------
# (2023-08-29, s-heppner)
# Enable color support of ls and add handy aliases. LS_COLORS itself is set by
# _bct_apply below (this only wires up the aliases and dircolors database).
...
_bct_apply "$(_bct_current)" || _bct_apply "$_BCT_DEFAULT_THEME"
